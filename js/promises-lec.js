...
// That's cool, but how do we practically use a promise? Well consider what we did back in AJAX:

// $('#submit').click(function (e) {
//     e.preventDefault();
//     $('#picture-example').html('<img src="img/Wedges-3s-200px.svg" alt="loading" class="kazuya">');
//     var val = $('input').val().toLowerCase();
//     $.ajax('https://pokeapi.co/api/v2/pokemon/' + val)
//         .done(function (resp) {
...
// This is cool, but that's relying on jQuery. With fetch, which is built into JavaScript, you won't need to include jQuery, potentially adding weight to our server/weighting for the CDN.

document.getElementById("submit").addEventListener("click", (e) => {
    e.preventDefault();
    document.getElementById("picture-example").innerHTML = '<img src="img/Wedges-3s-200px.svg" alt="loading" class="kazuya">';
    let val = document.getElementById("imgName").value;
fetch(`https://pokeapi.co/api/v2/pokemon/${val}`)
    ...
    .then(pokemon => {
        ...
        console.log(pokemon);
        document.getElementById("picture-example").innerHTML = `<img class="push" src="${pokemon.sprites.front_default}" alt="${val}"><h4>That's a Pokémon! Its name is: ${val}!</h4>`;
    })
    .catch(function () {
        document.getElementById("picture-example").innerHTML = `<img class="greymon" src="img/metal-greymon.gif" alt="${val} is not a Pokémon"><h4 class="pull">I'm sorry. "${val}" is not a Pokémon.</h4>`;
    });
});
...
// fetch('https://pokeapi.co/api/v2/pokemon/mewtwo').then(response => response.json()).then(data => {
